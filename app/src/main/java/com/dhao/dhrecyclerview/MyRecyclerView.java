...
public class MyRecyclerView extends RecyclerView {
    ...
    View headerView, footerView,noMoreView;
    ...
    private void changeHeight(int dy) {
        ...
        if (headerView.getLayoutParams().height>headerViewHeight*3){
            headerView.getLayoutParams().height=headerViewHeight*3;
        }
...
//        View refreshView = LayoutInflater.from(getContext()).inflate(R.layout.my_refresh, null);
        LinearLayout refreshView=new LinearLayout(getContext());
        refreshView.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        refreshView.setGravity(Gravity.BOTTOM|Gravity.CENTER_HORIZONTAL);
        refreshView.setPadding(0,40,0,40);
        status=new TextView(getContext());
        refreshView.addView(status);
...
//        status = (TextView) refreshView.findViewById(R.id.status);
        ...
        if (footers.size()==0) {
            ...
            LinearLayout noMoreLayout = new LinearLayout(getContext());
            noMoreLayout.setGravity(Gravity.CENTER);
            noMoreLayout.setLayoutParams(new ViewGroup.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
            footers.add(noMoreLayout);

            noMoreLayout.setPadding(0, 15, 0, 15);
            TextView text1 = new TextView(getContext());
            text1.setText("没有更多数据了");
            noMoreLayout.addView(text1);

            footerView = footerLayout;
            footerView.setVisibility(GONE);

            noMoreView=noMoreLayout;
            noMoreView.setVisibility(GONE);
        }else {
            footerView=footers.get(0);
            noMoreView=footers.get(1);
            footerView.setVisibility(GONE);
            noMoreView.setVisibility(GONE);
        }
    ...
    public void addFooterView(View loadingView,View noMoreView){
        footers.clear();
        footers.add(loadingView);
        if (noMoreView!=null){
