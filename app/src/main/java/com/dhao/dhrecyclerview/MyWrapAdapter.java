...
public class MyWrapAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MyWrapAdapter(RecyclerView.Adapter<RecyclerView.ViewHolder> adapter, ArrayList<View> headerViews, ArrayList footViews){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if(viewType== RecyclerView.INVALID_TYPE){
            ...
            for(int i=0;i<footViews.size();i++){
                return new RecyclerView.ViewHolder(footViews.get(i)){};
            }
        }
...
//            if (headerViews.size()-1<position & position<getItemCount()-footViews.size()-1){
